...
//  Got to figure out rules and permissions!  For now every time I load the driver I
// have to be root or change these permissions:
// sudo chmod 0444 /sys/class/usbmisc/osrfx2_0/device/switches
// sudo chmod 0666 /sys/class/usbmisc/osrfx2_0/device/7segment
// sudo chmod 0666 /sys/class/usbmisc/osrfx2_0/device/bargraph
// sudo chmod 0666 /dev/osrfx2_0
...
/*
http://learnlinuxconcepts.blogspot.com/2014/06/what-is-sysfs.html
...
static DEVICE_ATTR( switches, 0440, show_switches, NULL );

//static struct device_attribute dev_attr_switches = {
// .attr = {
//  .name = "switches",
//  .mode = S_IRUGO,
// },
...
static DEVICE_ATTR( bargraph, 0660, show_bargraph, set_bargraph );
...
static DEVICE_ATTR( 7segment, 0660, show_7segment, set_7segment );
...
//                            GFP_KERNEL, 
//                            &urb->transfer_dma );
    buf = usb_alloc_coherent( fx2dev->udev, 
                            count, 
                            GFP_KERNEL, 
                            &urb->transfer_dma );
    if (!buf) {
...
{
    struct osrfx2 * fx2dev;
// Relates to BKL Big Kernel Lock...
// Have to replace...
